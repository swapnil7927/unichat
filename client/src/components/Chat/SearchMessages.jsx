...
const SearchMessages = () => {
  ...
  const [{ currentChatUser, messages }, dispatch] = useStateContext();
  ...
  return (
    <div className="border-conversation-border border-l w-full bg-conversation-panel-background flex flex-col gap-4 z-10 max-h-screen ">
      ...
      <div className="overflow-auto custom-scrollbar h-full">
        <div className="flex items-center flex-col w-full">
          <div className="flex px-5 items-center gap-3 height-14 w-full">
          ...
          <div className="flex flex-col w-full h-full">
            {
              searchedMessages.map((message) => (
                <div className="flex cursor-pointer flex-col justify-center hover:bg-background-default-hover w-full px-5 border-b-[0.1px] border-secondary py-5">
                  <div className="text-small text-secondary">{calculateTime(message.createdAt)}
                  </div>
