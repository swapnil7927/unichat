...
const ChatListItem = ({ data, isContactsPage = false }) => {
  ...
  const [{ userInfo, currentChatUser, contactsPage }, dispatch] = useStateContext();
    ...
    if (!isContactsPage) {
      dispatch({
        ...
        user: {
          ...
          email: data.email,
          id: userInfo === data.senderId ? data.receiverId : data.senderId,
        },
      })
    } else {
      ...
      <div className="min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
        <div className="flex justify-between">
          ...
          </div>
          {
            !isContactsPage && (
              <div>
                <span className={`${!data.totalUnreadMessages > 0 ? "text-secondary" : "text-icon-green"} text-small`}>
        ...
        <div className="flex border-b border-conversation-border pb-2 pt-1 pr-2">
          <div className="flex justify-between w-full">
            ...
            {
              data.totalUnreadMessages > 0 && <span className="bg-icon-green px-[5px] justify-end rounded-full text-sm">{data.totalUnreadMessages}</span>
            }
          </div>
        </div>
      </div>
